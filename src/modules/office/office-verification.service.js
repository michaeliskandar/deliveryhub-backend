...
const FOLDER = CLOUDINARY_FOLDERS.OFFICE_VERIFICATION;
 ...
 * (e.g. national_id + commercial_register in a single submission). */
...
const reviewVerification = async (reviewerId, userId, { status, reviewNote }) => {
  ...
  const verification = await Verification.findOne({ user: userId });
  if (!verification) throw ApiError.notFound("No verification submission found for this office");
