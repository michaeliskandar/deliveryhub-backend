...
const FOLDER = CLOUDINARY_FOLDERS.CAPTAIN_VERIFICATION;
 ...
 * (e.g. national_id + driving_license + vehicle_license in one submission). */
...
const reviewVerification = async (reviewerId, userId, { status, reviewNote }) => {
  ...
  const verification = await Verification.findOne({ user: userId });
  if (!verification) throw ApiError.notFound("No verification submission found for this captain");
